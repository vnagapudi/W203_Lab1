...
```{r}
hist(cancer_data_fixed$MedianAge, main = "County Median Age Distribution", xlab = "Median Age")
```
...
```{r}
boxplot(cancer_data$PctWhite, cancer_data$PctBlack, cancer_data$PctAsian, cancer_data$PctOtherRace, cancer_data$PctWhite + cancer_data$PctBlack + cancer_data$PctAsian + cancer_data$PctOtherRace, names = c("White", "Black", "Asian", "Other", "Sum"))
```
...
```{r}
plot (~ PctPrivateCoverage + deathRate, data = cancer_data_fixed, col="red", xlab="% Coverage")
...
```{r}
lm(100*cancer_data_fixed$deathRate/mean(cancer_data_fixed$deathRate) ~ cancer_data_fixed$PctPrivateCoverage)
```
...
```{r}
scatterplotMatrix(~ cancer_data_fixed$povertyPercent + cancer_data_fixed$PctPrivateCoverage + cancer_data_fixed$PctPublicCoverage + cancer_data_fixed$PctHS25_Over + + cancer_data_fixed$PctBachDeg25_Over)
```
\newline
...
We can follow this by examining education directly in relation to death rate due to cancer. For this purpose we select two variables - percent of people with Bachelor's degrees and people with just High School education (over 25) and observe the correlation to Death rate. For the purposes of exploratory data analysis these are good indicators of the education level in the age groups with greatest cancer incidence.
```{r}
...
Curiously, there is a positive correlation between completion of high school and cancer death rate, but this most likely is created by the influence of aged populations and that nearly all people have completed high school - it is presumed that this variable is the percentage of population who are both high school graduates and over 25 and not those who are high school graduates given they are over 25. Therefore, this quantity is not useful for our analysis. The results coming from the inversion of the correlation seem intuitive for the case of those with a Bachelor's degree; if you have higher education, you tend to have better quality of life and economic position and hence cancer outcome. It could be considered also that higher education influences employement, and employment will result in private coverage. Let's examine the relationship between employment and education to be sure this is the case.
```{r}
