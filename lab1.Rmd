...
hist(cancer_data$PctPublicCoverage + cancer_data$PctPrivateCoverage)
```

Most private coverage is employee provided.

```{r}
hist(cancer_data$PctEmpPrivCoverage / cancer_data$PctPrivateCoverage)
```


c) The same problem exists with the race related information. It doesn't add up to 100%, however it is always less than 100, so it appears some people elected not to state their race. It appears Alaskan counties form the outliers - the predominant race must not be captured by this survey.
...
```{r}
boxplot(cancer_data$PctWhite, cancer_data$PctBlack, cancer_data$PctAsian, cancer_data$PctOtherRace)
